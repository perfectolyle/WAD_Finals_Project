    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto w-full max-w-3xl p-6">
                <h1 className="mb-6 text-2xl font-bold tracking-tight">Create Order</h1>
                <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-border bg-card p-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                            {lines.map((line, index) => (
                                <div key={index} className="flex items-center gap-3 rounded-lg border border-border p-3">
                                    <select
                                        value={line.id}
                                        onChange={(e) => updateLine(index, 'id', Number(e.target.value))}
                                    ...
                                    <input
                                        ...
                                        className="h-10 w-20 rounded-lg border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                                        placeholder="Qty"
                                    />
                    ...
                    <div className="flex items-center justify-between border-t border-border pt-4">
                        ...
                        <div className="flex gap-3">
                            ...
                            <Link href="/orders" className="inline-flex h-10 items-center rounded-lg border border-input px-6 text-sm font-medium transition-colors hover:bg-muted">Cancel</Link>
                        </div>
                    </div>
                </form>
            </div>
